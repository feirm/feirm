...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("FeirmMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("FeirmMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("FeirmMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("feirm-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrintf("Running FeirmMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
